...
def evaluation_1(config):
    ...
    metric = config['evaluation']['metric_1']
    label_list = config['evaluation']['label_list']
    label_fuse = config.get('label_fuse', False)
    organ_name = config['evaluation']['organ_name']
    gt_root    = config['evaluation']['ground_truth_folder_root']
    # seg_root   = config['segmentation_folder_root']
    # seg_root   = config['testing']['output_dir']
    output_dir = config['testing']['output_dir']
    ckpt_dir    = config['training']['ckpt_save_dir']
    ckpt_dir = ckpt_dir.split('/')[-1]
    subset = config['dataset']['test_csv']
    subset = subset.split('/')[-1][:-4]
    seg_root = os.path.join(output_dir,ckpt_dir+'_'+subset)

    if(not(isinstance(seg_root, tuple) or isinstance(seg_root, list))):
        seg_root = [seg_root]
    test_image_pair_csv = config['evaluation']['test_evaluation_image_pair']
    valid_image_pair_csv = config['evaluation']['valid_evaluation_image_pair']
    ...
    test_image_items = pd.read_csv(test_image_pair_csv)
    test_item_num = len(test_image_items)
    valid_image_items = pd.read_csv(valid_image_pair_csv)
    valid_item_num = len(valid_image_items)
    for seg_root_n in seg_root:
        score_all_data = []
        name_score_list= []
        for i in range(test_item_num):
            gt_name  = test_image_items.iloc[i, 0]
            seg_name = test_image_items.iloc[i, 1]
        ...
        score_csv = "{0:}/test_{1:}_{2:}_all.csv".format(seg_root_n, organ_name, metric)
        ...
        print("Test data: {0:} mean ".format(metric), score_mean)
        print("Test data: {0:} std  ".format(metric), score_std) 
        score_all_data = []
        name_score_list= []
        for i in range(valid_item_num):
            gt_name  = valid_image_items.iloc[i, 0]
            seg_name = valid_image_items.iloc[i, 1]
            # seg_name = seg_name.replace(".nii.gz", "_pred.nii.gz")
            gt_full_name  = gt_root  + '/' + gt_name
            seg_full_name = seg_root_n + '/' + seg_name
        ...
        # save the result as csv 
        score_csv = "{0:}/valid_{1:}_{2:}_all.csv".format(seg_root_n, organ_name, metric)
        with open(score_csv, mode='w') as csv_file:
            csv_writer = csv.writer(csv_file, delimiter=',', 
                            quotechar='"',quoting=csv.QUOTE_MINIMAL)
            head = ['image'] + ["class_{0:}".format(i) for i in label_list]
        ...
        print("Valid data: {0:} mean ".format(metric), score_mean)
        print("Valid data: {0:} std  ".format(metric), score_std) 

def evaluation_2(config):
    """
    Run evaluation of segmentation results based on a configuration dictionary `config`.
    The following fields should be provided in `config`:
    ...
    metric = config['evaluation']['metric_2']
    label_list = config['evaluation']['label_list']
    label_fuse = config.get('label_fuse', False)
    organ_name = config['evaluation']['organ_name']
    gt_root    = config['evaluation']['ground_truth_folder_root']
...
def eva_main(config):
    ...
    evaluation_2(config)
    print('done!')
    print(config['training']['ckpt_save_dir'])
    print(config['dataset']['test_csv'])
