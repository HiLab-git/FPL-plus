...
class NetRunAgent(object):
    ...
    def create_dataset(self):
        ...
        if(self.stage == 'train'):
            if self.config['network']['num_domains'] == 2: 
                self.train_set_1 = self.get_stage_dataset_from_config('1_train')
                self.train_set_2 = self.get_stage_dataset_from_config('2_train')
                self.valid_set_1 = self.get_stage_dataset_from_config('1_valid')
                self.valid_set_2 = self.get_stage_dataset_from_config('2_valid')
                if(self.deterministic):
                ...
                self.train_loader_1 = torch.utils.data.DataLoader(self.train_set_1, 
                    batch_size = bn_train, shuffle=True, num_workers= num_worker,
                    worker_init_fn=worker_init, generator = g_train)
                self.train_loader_2 = torch.utils.data.DataLoader(self.train_set_2, 
                    batch_size = bn_train, shuffle=True, num_workers= num_worker,
                    worker_init_fn=worker_init, generator = g_train)
                
                self.valid_loader_1 = torch.utils.data.DataLoader(self.valid_set_1, 
                    batch_size = bn_valid, shuffle=False, num_workers= num_worker,
                    worker_init_fn=worker_init, generator = g_valid)
                self.valid_loader_2 = torch.utils.data.DataLoader(self.valid_set_2, 
                    batch_size = bn_valid, shuffle=False, num_workers= num_worker,
                    worker_init_fn=worker_init, generator = g_valid)
                # self.dataloader_train.append(self.train_loader_1)
                # self.dataloader_train.append(self.train_loader_2)
                # self.dataloader_valid.append(self.valid_loader_1)
                # self.dataloader_valid.append(self.valid_loader_2)
            elif self.config['network']['num_domains'] == 1: 
                self.train_set = self.get_stage_dataset_from_config('1_train')
                self.valid_set = self.get_stage_dataset_from_config('1_valid')
                if(self.deterministic):
                    def worker_init_fn(worker_id):
                        # workder_seed = self.random_seed+worker_id 
                        workder_seed = torch.initial_seed() % 2 ** 32
                ...
                bn_train = self.config['dataset']['train_batch_size']
                bn_valid = self.config['dataset'].get('valid_batch_size', 1)
                num_worker = self.config['dataset'].get('num_workder', 32)
                g_train, g_valid = torch.Generator(), torch.Generator()
                g_train.manual_seed(self.random_seed)
                g_valid.manual_seed(self.random_seed)
                self.train_loader_1 = torch.utils.data.DataLoader(self.train_set, 
                    batch_size = bn_train, shuffle=True, num_workers= num_worker,
                    worker_init_fn=worker_init, generator = g_train)
                self.valid_loader_1 = torch.utils.data.DataLoader(self.valid_set, 
                    batch_size = bn_valid, shuffle=False, num_workers= num_worker,
                    worker_init_fn=worker_init, generator = g_valid)
                # self.dataloader_train.append(self.train_loader_1)
                # self.dataloader_valid.append(self.valid_loader_1)
