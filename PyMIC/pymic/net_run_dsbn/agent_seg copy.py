...
class SegmentationAgent(NetRunAgent):
    ...
    def get_stage_dataset_from_config(self, stage):
        ...
        if self.train_fpl_uda:
            dataset  = NiftyDataset(root_dir  = root_dir,
                                    csv_file  = csv_file,
                                    modal_num = modal_num,
                                    train_fpl_uda = self.train_fpl_uda,
    ...
    def get_loss_value(self, data, pred, gt, param = None):
        ...
        if data.get('pixel_weight', None) is not None:
            ...
            # print('111',data['pixel_weight'].shape,data['image_weight'],self.loss_calculator)
    ...
    def training_dual_doamian(self):
        ...
        for it in range(iter_valid):
            ...
            inputs_1 = self.convert_tensor_type(data_0['pixel_weight'])
            inputs_1 = self.convert_tensor_type(inputs_1).to(self.device)
            inputs_0 = self.convert_tensor_type(data_0['image'])
            sample_weight = data_0['image_weight']
            ...
            outputs_11 = self.net(inputs_0, domain_label=1*torch.ones(inputs_0.shape[0], dtype=torch.long))
            outputs_22 = self.net(inputs_1, domain_label=0*torch.ones(inputs_1.shape[0], dtype=torch.long))
            print(inputs_0.max(),inputs_0.min(),inputs_1.max(),inputs_1.min(),'****************************************')
            print('sample weight:',sample_weight)
            if sample_weight[0] < 0.23:
                loss_12 = self.get_loss_value(data_0, outputs_11, labels_prob_0,self.fpl_uda)
                loss_11 = self.get_loss_value(data_0, outputs_22, labels_prob_0,self.fpl_uda)
                loss = (loss_11+loss_12)
                print('sample weight < 0.23:',sample_weight,loss)
            else:
                loss_2 = self.criterionL2(outputs_11,outputs_22)
                loss = loss_2
                print('sample weight > 0.23:',sample_weight,loss)
            loss.backward()
            self.optimizer.step()
            ...
            soft_out_11, labels_prob = reshape_prediction_and_ground_truth(soft_out_11, labels_prob_0) 
            dice_list_11 = get_classwise_dice(soft_out_11, labels_prob)
            train_dice_list_0.append(dice_list_11.cpu().numpy())
        
            soft_out_22, labels_prob = reshape_prediction_and_ground_truth(soft_out_22, labels_prob_0) 
            dice_list_22 = get_classwise_dice(soft_out_22, labels_prob)
            train_dice_list_1.append(dice_list_22.cpu().numpy())  
            # if self.config['training']['dis']  == True:
            #     # self.optimizer.zero_grad()
    ...
    def train_valid(self):
        ...
        for it in range(iter_start, iter_max, iter_valid):
            ...
            print(self.dual)
            if self.dual:
                train_scalars = self.training_dual_doamian()
                valid_scalars = self.validation_dual()
            else:
    ...
    def infer(self):
            ...
            for data in self.test_loader:
                ...
                if self.FPL:
                    for i in range(6):
                        ...
                        pred = self.inferer.run_flip(self.net, images,i, domain_label=domian_label*torch.ones(images.shape[0], dtype=torch.long))
                    ...
                    # for T2:
                    # sample_wise_weight = -1.557408*((2.0*(uncer_one-0.1265)/(0.3766-0.1265))-1.0)
                    # for T1-T2:
                    # sample_wise_weight = -1.557408*((2.0*(uncer_one-0.0763)/(0.4-0.0763))-1.0)
                    
                    # # print((np.arctan(sample_wise_weight)+1)/2,'506',names)
                    # sample_wise_weight = np.abs((np.arctan(sample_wise_weight)+1)/2)
                    ...
                    # # if save label with soft:
                    # pred =  np.expand_dims(np.mean(maps,axis=0),0)
                    # # if save label with hard:
                    # pred = np.asarray(np.argmax(pred,  axis = 1), np.uint8)
                    # if(self.postprocessor is not None):
                    #     for i in range(len(names)):
                    #         pred[i] = self.postprocessor(pred[i])
                    # # print(pred.shape) # (1, 30, 160, 272) 
                    # data['predict'] = pred
                    # data['pixel_wise_weight'] = pixel_wise_weight  # (30,160,272)
                    # data['sample_wise_weight'] = sample_wise_weight
                    # root_dir  = self.config['testing']['output_dir']
                    # for i in range(len(names)):
                    #     # print(names,names[0],'525',root_dir + '/' + names[i].split('/')[-1].split('.')[0])
                    #     np.save(root_dir + '/' + names[i].split('/')[-1].split('.')[0],data)
        
                    # uncertainty_list.append(uncer_one)
                    print(names[0],uncer_one)
                    uncertainty_list[names[0]] = [uncer_one]
                    
                    # print(names,vars,boundary,vars.max(),uncer_one,'477',uncertainty_list,'**')
                    # uncertainty_list[uncertainty_list == 0] = (max(uncertainty_list))
                    # print(uncertainty_list,max(uncertainty_list),min(uncertainty_list))
        ...
        if self.FPL:
            ...
            np.save('script/FPL/vs_uncertainty_t2_model_twostage_flip.npy', student_tuplelist_sorted)
            np.save('script/FPL/vs_uncertainty_t2_model_pixel_weight_flip.npy', pixel_weight)
